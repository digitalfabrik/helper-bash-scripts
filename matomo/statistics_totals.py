...
def plot(region, regions, stats, month, tempdir):
    """
    Plot graph
    """
    if ARGS.verbose:
...
def dump_data(region, regions, stats, month, tempdir):
    """
    ...
    with open(filename, "w") as csv_file:
        csv_file.write("date,{}\n".format(','.join(regions[region]["languages"])))
...
def main():
    ...
    for region in regions:
        ...
        file_list.append(plot(region, regions, stats, month, tempdir))
        file_list.append(dump_data(region, regions, stats, month, tempdir))
...
PARSER.add_argument("--matomo-url", "URL to Matomo")
